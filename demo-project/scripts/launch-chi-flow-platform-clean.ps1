...
function Test-ChiEnergyFlow {
    ...
    $energyTests = @(
        @{ Channel = "IDE"; Port = 3000; Path = "/api/health"; Symbol = "IDE" },
        @{ Channel = "Dashboard"; Port = 5000; Path = "/health"; Symbol = "DASH" },
        @{ Channel = "MainApp"; Port = 3002; Path = "/api/health"; Symbol = "MAIN" },
        @{ Channel = "WebSocket"; Port = 3002; Path = $null; Symbol = "WS" }
    )
...
function Show-ChiFlowDashboard {
    ...
    $principles = @(
        ...
        "Background Processing: Invisible wind energy constant",
        ...
        "Continuous Power: Solar energy never depleting"
    )
    ...
    Write-Host "   curl -X POST http://localhost:5000/api/arbitrate" -ForegroundColor Gray
    Write-Host "" -ForegroundColor Gray
